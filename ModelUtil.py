...
from tensorflow.keras.applications import ResNet50, ResNet101, VGG16, VGG19, DenseNet121, Xception
...
def create_BiT_model(input_shape, num_classes):
    model = tf.keras.Sequential([
        tf.keras.layers.InputLayer(input_shape=input_shape),
        hub.KerasLayer("https://tfhub.dev/google/bit/s-r50x1/1", trainable=True),
        #tf.keras.layers.Dropout(rate=0.2),
        Dense(num_classes, use_bias=True, kernel_initializer='zeros', trainable=True),
        Activation("softmax")
    ])
    model.summary()
    return model
